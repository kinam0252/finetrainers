...
class WanTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        target_frame_mask: torch.Tensor = None,
        zero_temb: torch.Tensor = None,
    ) -> torch.Tensor:
        shift_msa, scale_msa, gate_msa, c_shift_msa, c_scale_msa, c_gate_msa = (
            self.scale_shift_table + temb.float()
        ).chunk(6, dim=1)
        if apply_target_noise_only:
            zero_shift_msa, zero_scale_msa, zero_gate_msa, zero_c_shift_msa, zero_c_scale_msa, zero_c_gate_msa = (
                self.scale_shift_table + zero_temb.float()
            ).chunk(6, dim=1)

        # 1. Self-attention
        if apply_target_noise_only:
            zero_norm_hidden_states = (self.norm1(hidden_states.float()) * (1 + zero_scale_msa) + zero_shift_msa).type_as(hidden_states)
            norm_hidden_states = (self.norm1(hidden_states.float()) * (1 + scale_msa) + shift_msa).type_as(hidden_states)
            norm_hidden_states = torch.where(target_frame_mask.unsqueeze(-1), zero_norm_hidden_states, norm_hidden_states)
        else:
            norm_hidden_states = (self.norm1(hidden_states.float()) * (1 + scale_msa) + shift_msa).type_as(hidden_states)
        attn_output = self.attn1(hidden_states=norm_hidden_states, rotary_emb=rotary_emb)
        if apply_target_noise_only:
            zero_hidden_states = (hidden_states.float() + attn_output * zero_gate_msa).type_as(hidden_states)
            hidden_states = (hidden_states.float() + attn_output * gate_msa).type_as(hidden_states)
            hidden_states = torch.where(target_frame_mask.unsqueeze(-1), zero_hidden_states, hidden_states)
        else:
            hidden_states = (hidden_states.float() + attn_output * gate_msa).type_as(hidden_states)
        ...
        # 3. Feed-forward
        if apply_target_noise_only:
            zero_norm_hidden_states = (self.norm3(hidden_states.float()) * (1 + zero_c_scale_msa) + zero_c_shift_msa).type_as(hidden_states)
        ...
        if apply_target_noise_only:
            zero_hidden_states = (hidden_states.float() + ff_output.float() * zero_c_gate_msa).type_as(hidden_states)
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    ...
    def forward(
        ...
        if apply_target_noise_only:
            zero_shift, zero_scale = (self.scale_shift_table + zero_temb.unsqueeze(1)).chunk(2, dim=1)
            zero_shift = zero_shift.to(hidden_states.device)
            zero_scale = zero_scale.to(hidden_states.device)
            zero_hidden_states = (self.norm_out(hidden_states.float()) * (1 + zero_scale) + zero_shift).type_as(hidden_states)
