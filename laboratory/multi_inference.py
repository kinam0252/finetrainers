...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["front-4-none", "front-4-noise-none", "front-4-sdedit-none", "None"],
    )
...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        for i, prompt in enumerate(prompts[:args.num_videos]):
            for apply_mode in args.apply_target_noise_only:
                if apply_mode == "None":
                    apply_mode = None
                savedir = f"laboratory/outputs/{apply_mode}"
                os.makedirs(savedir, exist_ok=True)
                save_video_path = os.path.join(savedir, f"output_{i}.mp4")
                if os.path.exists(save_video_path):
                    print(f"skip {save_video_path} because it already exists")
                    continue
                ...
                if apply_mode == "front-4-sdedit-none":
                    input_latents = None
                else:
                ...
                video = pipe(prompt, 
                            ...
                            apply_target_noise_only=apply_mode,
