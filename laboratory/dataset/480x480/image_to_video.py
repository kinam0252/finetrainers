import os
import subprocess
import tempfile
from PIL import Image

def create_video_from_image(image_path, output_path, num_frames=49, fps=10):
    # Create temporary directory
    with tempfile.TemporaryDirectory() as temp_dir:
        # Load and resize image
        original_image = Image.open(image_path)
        original_image = original_image.resize((480, 480), Image.LANCZOS)
        
        # Save the same image multiple times as frames
        for i in range(num_frames):
            frame_path = os.path.join(temp_dir, f'frame_{i:04d}.jpg')
            original_image.save(frame_path)
        
        # Create video from frames
        concat_cmd = [
            'ffmpeg',
            '-framerate', str(fps),
            '-i', os.path.join(temp_dir, 'frame_%04d.jpg'),
            '-c:v', 'libx264',
            '-pix_fmt', 'yuv420p',
            '-preset', 'medium',
            '-crf', '23',
            '-y',
            output_path
        ]
        subprocess.run(concat_cmd)

def main():
    # Input and output directories
    input_dir = '/data/kinamkim/finetrainers/laboratory/dataset/480x480/images'
    output_dir = 'videos'
    
    # Create output directory if it doesn't exist
    os.makedirs(output_dir, exist_ok=True)
    
    # Get all image files and sort them
    image_files = sorted([f for f in os.listdir(input_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg'))])
    
    # Process each image
    for i, image_file in enumerate(image_files, 1):
        print(f"Processing {image_file}...")
        
        # Input and output paths
        image_path = os.path.join(input_dir, image_file)
        output_path = os.path.join(output_dir, f'{i}.mp4')
        
        # Create video from image
        create_video_from_image(image_path, output_path)
        print(f"Created video: {output_path}")

if __name__ == "__main__":
    main() 